...
export default function ProductViewPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    fetchProductDetail(params.id).then((detail) => {
      // Products with the landing page toggle ON use the full sales
      // page at /p/[slug] instead of this plain browsing view.
      if (detail?.landingPageActive) {
        router.replace(`/p/${params.id}`);
        return;
      }

      setProduct(detail);
      setNotFoundFlag(!detail);
      setActiveImage(0);
      setQuantity(1);
  ...
  if (notFoundFlag) {
    return (
      <div className="py-20 text-center text-brand-muted font-bold space-y-3">
        <p>{locale === 'bn' ? 'প্রোডাক্টটি খুঁজে পাওয়া যায়নি।' : 'Product not found.'}</p>
        <Link href="/shop" className="text-brand-primary underline">
          {locale === 'bn' ? 'শপে ফিরে যান' : 'Back to Shop'}
        </Link>
      </div>
    );
  }
