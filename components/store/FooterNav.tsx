...
import { Home, ShoppingCart, Crown, Sparkles, Gift, Shirt, LayoutGrid, Info, Truck, RefreshCw, ShieldAlert, Phone, MapPin, PhoneCall, Mail, User, PackageSearch } from 'lucide-react';
...
export default function FooterNav() {
  ...
  /* ── Mobile bottom nav items: Home/Shop/Cart/Live Chat/Account ── */
  ...
  const mobileNav: MobileNavItem[] = [
    ...
    { label_en: 'Shop',      label_bn: 'শপ',         icon: LayoutGrid,   href: `/shop` },
    { label_en: 'Cart',      label_bn: 'কার্ট',       icon: ShoppingCart, action: 'cart' },
    { label_en: 'Live Chat', label_bn: 'লাইভ চ্যাট',  icon: WhatsAppIcon, href: `/live-chat` },
          ...
          <div className="md:col-span-1 space-y-5">
            ...
            <Link href="/" className="flex items-center group w-fit">
              <img
                src="/logo.svg"
                alt="Redaa"
                className="h-10 w-auto object-contain group-hover:scale-105 transition-transform duration-200 invert"
              />
            </Link>

            <p className="text-[#F2EDE3]/50 text-xs leading-relaxed max-w-[220px]">
              {locale === 'bn'
                ? 'বাংলাদেশের সেরা প্রিমিয়াম পোশাক ও লাইফস্টাইল ব্র্যান্ড।'
                : "Bangladesh's finest premium clothing and lifestyle brand."}
            </p>

            {/* Social icons */}
            <div className="flex gap-3">
              {/* Facebook */}
              <a href="https://www.facebook.com/redaabangladesh" target="_blank" rel="noopener noreferrer" className="h-8 w-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-[#F2EDE3]/60 hover:bg-[#C6A15B] hover:text-[#14201D] hover:border-[#C6A15B] transition-all duration-200">
              ...
              {/* Instagram */}
              <a href="https://www.instagram.com/redaabangladesh" target="_blank" rel="noopener noreferrer" className="h-8 w-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-[#F2EDE3]/60 hover:bg-[#C6A15B] hover:text-[#14201D] hover:border-[#C6A15B] transition-all duration-200">
              ...
              {/* WhatsApp */}
              <a href="https://wa.me/8801970452723" target="_blank" rel="noopener noreferrer" className="h-8 w-8 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-[#F2EDE3]/60 hover:bg-[#C6A15B] hover:text-[#14201D] hover:border-[#C6A15B] transition-all duration-200">
          ...
          <div className="hidden md:block space-y-4">
            ...
            <ul className="space-y-2.5">
              {[
                ...
                { en: 'Panjabi',        bn: 'পাঞ্জাবি',     icon: Shirt, href: `/shop?category=panjabi` },
                { en: 'Tupi',           bn: 'টুপি',        icon: Crown, href: `/shop?category=tupi` },
                { en: 'Rumal',          bn: 'রুমাল',       icon: Gift, href: `/shop?category=rumal` },
                { en: 'Orna',           bn: 'ওড়না',        icon: Sparkles, href: `/shop?category=orna` },
              ].map((l, i) => {
                const Icon = l.icon;
                return (
                  <li key={i}>
          ...
          <div className="space-y-5">
            ...
            <ul className="space-y-3 text-xs text-[#F2EDE3]/55">
              ...
              <li className="flex items-center gap-2.5">
                ...
                <span>01970452723</span>
              </li>
              <li className="flex items-center gap-2.5">
                <span className="h-7 w-7 rounded-full bg-white/5 border border-white/10 flex items-center justify-center flex-shrink-0">
                  <Mail className="h-3.5 w-3.5 text-[#C6A15B]" strokeWidth={1.75} />
                </span>
                <span>redaabangladesh@gmail.com</span>
              </li>
            </ul>
          </div>
        ...
        <div className="py-4">
          <div className="max-w-7xl mx-auto px-6 lg:px-8 flex flex-col items-center gap-2">
            ...
            <p className="text-[11px] text-[#F2EDE3]/35">
              © 2026 Redaa. {locale === 'bn' ? 'সর্বস্বত্ব সংরক্ষিত।' : 'All rights reserved.'}
            </p>
          </div>
        </div>
      </footer>
